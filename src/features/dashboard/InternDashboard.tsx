...
export const InternDashboard = () => {
  ...
  const stats = [
    { label: 'Active Projects', value: '2', icon: FolderKanban, trend: 'On track', trendUp: true },
    { label: 'Upcoming Events', value: '3', icon: Calendar, trend: 'Next: Friday', trendUp: true },
    { label: 'Upcoming Trips', value: '1', icon: Plane, trend: 'Zurich, CH', trendUp: true },
    { label: 'Pending Tasks', value: '8', icon: CheckSquare, trend: '3 due today', trendUp: false },
  ];
    ...
    <div className="space-y-6 pb-10">
      ...
      <div className="bg-gradient-to-r from-[#04376C] to-[#0A4D8C] rounded-2xl p-8 text-white shadow-[0_10px_30px_rgba(4,55,108,0.15)] flex flex-col md:flex-row justify-between items-start md:items-center relative overflow-hidden">
        <div className="relative z-10">
          <h1 className="text-3xl font-bold tracking-tight mb-2">Welcome Back, {user?.name?.split(' ')[0] || 'Intern'}!</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="flex items-center justify-between mb-2">
            <h2 className="text-xl font-bold text-[#1F2937]">Current Projects</h2>
            <Button variant="ghost" size="sm" className="text-[#04376C]">View All</Button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {[
              ...
              <Card key={i} hoverEffect className="p-6 flex flex-col border border-[#E5E7EB]">
                <div className="flex justify-between items-start mb-4">
                  <Badge variant={project.status === 'In Progress' ? 'info' : 'warning'}>
                    {project.status}
                  </Badge>
        ...
        <div className="lg:col-span-1 space-y-6">
          <Card className="p-6 border border-[#E5E7EB] h-full">
            ...
            <div className="space-y-6">
              {[
                { title: 'Weekly Mentor Meeting', desc: 'Sync with Dr. Sarah Jenkins', time: 'Tomorrow, 10:00 AM', type: 'meeting', color: 'bg-[#1E6FD9]' },
                { title: 'Zurich Tech Hub Visit', desc: 'IAESTE Local Committee Trip', time: 'Friday, 08:00 AM', type: 'event', color: 'bg-[#F59E0B]' },
                { title: 'Project Proposal Due', desc: 'Phase 2 Architecture Document', time: 'Oct 20, 11:59 PM', type: 'deadline', color: 'bg-[#EF4444]' }
              ].map((item, i) => (
                <div key={i} className="flex relative pl-5">
                  <div className={`absolute left-0 top-1.5 w-2.5 h-2.5 rounded-full ${item.color} shadow-sm`}></div>
                  ...
                  <div>
                    ...
                    <p className="text-xs font-medium text-[#04376C] mt-0.5">{item.desc}</p>
            ...
            <Button variant="secondary" className="w-full mt-8">View Full Calendar</Button>
          </Card>
        </div>
